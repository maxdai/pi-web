...
- **扩展对话框的断线加固（补充决策，2026-09）**：Pi 的 TUI/RPC 在对话框上无限等待（用户在终端前 / RPC 客户端自带超时），而浏览器标签页可能在 agent loop 运行中被关闭——此时 `select/confirm/input/editor` 的请求广播给零个客户端、promise 永不 settle，**整个 loop 卡死**（例：aft 的宿主机执行确认）。三层加固：
  - **默认超时（分档）**：扩展自带 `opts.timeout`（毫秒）优先；否则按"是否有浏览器连接"分档——**有客户端 10 分钟**（用户可能离开）、**无客户端 60 秒**（关掉标签页不应长期堵塞 loop，但页面刷新/重连需要一点宽限）。超时以对话框默认值 settle（confirm→false，其余→undefined）。
  ...
  - **浏览器侧同步关闭**：服务端 settle（超时/abort/会话切换）后广播 `dialog_dismissed`（id+reason），前端关闭对应 modal 并追加一条警告行（`.notify-line`），避免留下可点击但已失效的弹窗；`clearSessionState` 亦广播（会话切换时 `clearContent()` 不会关弹窗）。默认超时值可由 `WebUIContextOptions` 覆盖（`dialogTimeoutMs`/`dialogTimeoutNoClientMs`，测试与调优用）。
