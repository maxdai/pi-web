...
- **usage 命令的专用数据面板（Web 增强，补充决策）**：`/usage`（pi-usage-extension）是 custom-only 扩展（数据收集与面板全在 `ctx.ui.custom()` 的 TUI 组件内，Web 无法渲染）。方案：pi-web **真实执行** /usage 命令（custom() stub 调 factory → 扩展后台 collect 更新 ~/.pi/agent/usage-extension-cache.json）→ 服务端（usage-render.ts 独立模块）**读 cache 聚合出结构化数据**（5 个时间段 × provider/model × 7 指标 + 简版 insights + 全局 hourly 序列 + tabWindow 时间窗口元数据）→ 广播专用消息 `usage_data`（**非 notify 文本**）→ 前端 **独立大面板**（非普通 modal）渲染：时间段 tab、Table 视图（provider 行可展开模型明细 + totals）、Insights 视图、Graph 视图（CSS 柱状图——hourly 趋势 + provider 占比）。**职责划分原则（2026-09-02 用户提出并确认）**：服务端只提供**基础数据**（聚合统计 + 全局时间序列 + tab 时间窗口元数据），**一切渲染适配在前端完成**——按 tab 过滤 hourly、补 0 连续时间轴、聚合列数、坐标标签等都不进服务端（曾误把 hourlyByTab 逐 tab 分桶放服务端，属渲染需求，已回退为前端过滤）。缓存格式为扩展私有（version 7：names/files，message 13 元组），聚合逻辑独立于扩展实现（usage-render.ts），扩展版本变化时只需调整该模块。
