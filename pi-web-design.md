...
- **主题切换（dark/light 对齐 TUI，补充决策）**：命名对齐 TUI——前端主题为 **dark / light**（原 bright 更名；CSS 色值本来就对齐 Pi light.json，`body.theme-light` 类名）。切换闭环：前端点主题按钮 → 发 `set_theme` WS 消息（`{name: "dark"|"light"}`）→ 服务端 `WebUIContext.setTheme(name)` 换 Theme 实例（加载对应 `dark.json`/`light.json` 色值）→ 前端 `location.reload()` 刷新页面（CSS 变量即时生效 + 扩展 ANSI 色重渲染）。**生效方式**：切换即刷新（TUI 是即时整体重绘，web 无此机制，刷新页面等价且简单可靠；切换时页面闪一次可接受）。
